...
impl TradeTable {
    ...
    pub fn standard() -> Result<Self, String> {
        ...
        table.load_from_data(STANDARD_TRADE_GOODS)?;
        Ok(table)
    }

    /// Load trade goods from embedded data array
    pub fn load_from_data(&mut self, data: &[&[&str; 7]]) -> Result<&mut Self, String> {
...
impl TradeTableEntry {
    pub fn from_string_with_line(
        ...
        if index_str.len() != 2 {
            return Err(format!("Line {}: Index must be 2 digits, got {}", line_num, index_str));
        }
        ...
        {
            return Err(format!(
                "Line {}: Index digits must be between 1-6, got {}",
                line_num, index_str
            ));
        }
        ...
        let availability = if availability_str == "All" {
            ...
            for code in codes {
                if let Some(tc) = crate::string_to_trade_class(code) {
                    trade_classes.push(tc);
                } else {
                    return Err(format!("Line {} (Index {}): Invalid trade class code: {}", 
                        line_num, index, code));
                }
            }
        ...
        if parts.len() != 3 {
            return Err(format!(
                "Line {} (Index {}): Quantity must be in format nDxM, got {}",
                line_num, index, quantity_str
            ));
        }

        let dice = parts[0]
            .parse::<u8>()
            .map_err(|_| format!("Line {} (Index {}): Invalid dice count: {}", 
                line_num, index, parts[0]))?;
        let multiplier = parts[2]
            .parse::<i16>()
            .map_err(|_| format!("Line {} (Index {}): Invalid multiplier: {}", 
                line_num, index, parts[2]))?;

        let quantity = Quantity { dice, multiplier };

        // Parse base cost
        let base_cost = base_cost_str
            .parse::<i32>()
            .map_err(|_| format!("Line {} (Index {}): Invalid base cost: {}", 
                line_num, index, base_cost_str))?;
...
fn parse_dm_string_with_line(dm_str: &str, line_num: usize, index: Option<i16>) -> Result<HashMap<TradeClass, i16>, String> {
    ...
    for part in parts {
        ...
        let sign_pos = part.find(|c| c == '+' || c == '-')
            .ok_or_else(|| {
                if let Some(idx) = index {
                    format!("Line {} (Index {}): No + or - found in DM: {}", line_num, idx, part)
                } else {
                    format!("Line {}: No + or - found in DM: {}", line_num, part)
                }
            })?;
        ...
        let trade_class = crate::string_to_trade_class(code)
            .ok_or_else(|| {
                if let Some(idx) = index {
                    format!("Line {} (Index {}): Invalid trade class code: {}", line_num, idx, code)
                } else {
                    format!("Line {}: Invalid trade class code: {}", line_num, code)
                }
            })?;
        
        let dm = dm_str.parse::<i16>()
            .map_err(|_| {
                if let Some(idx) = index {
                    format!("Line {} (Index {}): Invalid DM value: {}", line_num, idx, dm_str)
                } else {
                    format!("Line {}: Invalid DM value: {}", line_num, dm_str)
                }
            })?;
