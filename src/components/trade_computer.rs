...
//! - Ship Broker skill affects purchase prices
...
//! - **Buy Prices**: Base cost modified by origin world and Ship Broker skill
...
//! ├── Ship Section
//! │   ├── Ship Broker skill / Steward skill
//! │   ├── Crew size / Crew staterooms
//! │   ├── Passenger staterooms / Low berths / Cargo capacity
//! │   ├── Mortgage / Maintenance / Salary per period
//! │   └── Monthly expenses readout + Apply button
//! ├── Trade Leg Section
//! │   ├── Distance Input (manual override)
//! │   ├── System Broker skill
//! │   ├── Include Illegal Goods
//! │   └── Origin / Destination trade-class readouts
...
/// localStorage key for remembering which ship the user was last viewing.
/// Read on mount, written whenever they pick a different ship.
const SHIP_NAME_STORAGE_KEY: &str = "worldgen.trade.ship_name";

/// Read the saved ship name from `localStorage` (returns the trimmed
/// non-empty value, or None).
fn load_saved_ship_name() -> Option<String> {
    let storage = web_sys::window()?.local_storage().ok()??;
    let raw = storage.get_item(SHIP_NAME_STORAGE_KEY).ok()??;
...
/// Persist the active ship name so it sticks across page reloads.
fn save_ship_name(name: &str) {
    if let Some(window) = web_sys::window()
        && let Ok(Some(storage)) = window.local_storage()
        && let Err(e) = storage.set_item(SHIP_NAME_STORAGE_KEY, name)
    {
        log::warn!("Failed to save ship name to localStorage: {:?}", e);
    }
}
...
pub fn Trade(
    ...
    // Ship name is the *first* thing the user enters: we key all persisted
    // trade data by this name on the server, and we disable every other
    // input until it's committed. We seed `current_ship` from
    // localStorage so returning users land on their last ship; they can
    // type a new name in `ship_name_input` and click "Set Ship" to
    // switch.
    let saved_ship = load_saved_ship_name();
    let ship_name_input = RwSignal::new(saved_ship.clone().unwrap_or_default());
    let current_ship = RwSignal::new(saved_ship);
    ...
    // costs, ship's broker / steward skill). Server-authoritative once
    // persisted; clients edit fields locally and the server echoes the
    // merged state back. `ship.name` is the canonical store for the
    // ship's display name.
    let (ship, write_ship) = signal::<Ship>(Ship::default());

    // Planet-side broker skill on the current trade leg. Per-trade-leg
    // (NOT a ship attribute) — the player meets a different system
    // broker every time they jump to a new world.
    let (system_broker_skill, write_system_broker_skill) = signal::<i16>(0);
    ...
    // If we have a saved ship name, queue it for the client so the
    // server hands us that ship's session as soon as the WS opens. The
    // client buffers it and `onopen` flushes it. We do this *after*
    // signal creation but *before* register_signals so that any reply
    // landing post-registration is handled correctly. (The Client's
    // own onopen happens on a later async tick, so signals will always
    // be registered by then in practice.)
    if let (Some(c), Some(name)) = (client.as_ref(), current_ship.get_untracked()) {
        c.set_ship(name);
    }
    ...
    if let Some(ref client) = client {
        let signals = TradeSignals {
            ...
            ship: write_ship,
            system_broker_skill: write_system_broker_skill,
        ...
        Effect::new(move |_| {
            ...
            let current_ship = ship.get();
            let current_system_broker_skill = system_broker_skill.get();
            ...
            let state = TradeState {
                ...
                system_broker_skill: current_system_broker_skill,
    ...
    // The Set Ship handler needs its own clone of the client (the
    // Regenerate on:click below moves `client` into its closure, so we
    // can't share). Committing a new ship: persist to localStorage,
    // flip `current_ship`, and tell the server which session to load.
    let client_for_set_ship = client.clone();
    let commit_ship = move || {
        let trimmed = ship_name_input.get_untracked().trim().to_string();
        if trimmed.is_empty() {
            return;
        }
        save_ship_name(&trimmed);
        current_ship.set(Some(trimmed.clone()));
        if let Some(ref c) = client_for_set_ship {
            c.set_ship(trimmed);
        }
    };
    let commit_ship_click = commit_ship.clone();
    let commit_ship_key = commit_ship;

    let ship_set = Memo::new(move |_| current_ship.with(|s| s.is_some()));

    // Two clones of the client are kept here because the Regenerate and
    // Apply Monthly Expenses on:click closures both move the captured
    // client; without these we'd hit "use of moved value".
    let client_for_regenerate = client.clone();
    let client_for_apply = client.clone();
    ...
    view! {
        <div class:App>
            ...
            <div class="key-region world-entry-form">
                <label class="ship-name-label">
                    ...
                    <input
                        ...
                        prop:value=move || ship_name_input.get()
                        on:input=move |ev| ship_name_input.set(event_target_value(&ev))
                        on:keydown=move |ev: leptos::ev::KeyboardEvent| {
                            if ev.key() == "Enter" {
                                commit_ship_key();
                            }
                        }
                        placeholder="Enter ship name to begin"
                    />
                </label>
                <button
                    class="blue-button"
                    on:click=move |_| commit_ship_click()
                    disabled=move || {
                        let name = ship_name_input.with(|s| s.trim().to_string());
                        name.is_empty() || current_ship.with(|cur| cur.as_deref() == Some(&name))
                    }
                >
                    "Set Ship"
                </button>
                {move || {
                    current_ship
                        .get()
                        .map(|s| view! {
                            <span class="ship-name-current">"Current ship: " {s}</span>
                        })
                }}
            </div>
            <Show when=move || !ship_set.get()>
                <div class="ship-name-prompt">
                    "Enter a ship name above and click "
                    <strong>"Set Ship"</strong>
                    " to begin. All trade data — origin and destination worlds, "
                    "skills, available goods, passengers, freight, and your ship "
                    "manifest — will be saved under that ship name and restored "
                    "automatically when you return."
                </div>
            </Show>
            <fieldset
                class="trade-fieldset"
                style="border: none; padding: 0; margin: 0;"
                disabled=move || !ship_set.get()
            >
            // Ship configuration: capacity, crew, hardware, and periodic
            // costs. Distinct from the per-trade-leg section below — these
            // are properties of the ship itself, not of the current leg.
            <div class:key-region>
                <h3 class="trade-section">"Ship"</h3>
                <div class="skill-entry">
                    <div>
                        ...
                        <input
                            ...
                            min="0"
                            max="100"
                            prop:value=move || ship.with(|s| s.broker_skill)
                            on:change=move |ev| {
                                let v: i16 = event_target_value(&ev).parse().unwrap_or(0).max(0);
                                write_ship.update(|s| s.broker_skill = v);
                            }
                        />
                    </div>
                    <div>
                        ...
                        <input
                            ...
                            min="0"
                            max="100"
                            prop:value=move || ship.with(|s| s.steward_skill)
                            on:change=move |ev| {
                                let v: i16 = event_target_value(&ev).parse().unwrap_or(0).max(0);
                                write_ship.update(|s| s.steward_skill = v);
                            }
                        />
                    </div>
                    <div>
                        ...
                        <input
                            ...
                            id="ship-crew-size"
                            min="0"
                            prop:value=move || ship.with(|s| s.crew_size)
                            on:change=move |ev| {
                                let v: i32 = event_target_value(&ev).parse().unwrap_or(0).max(0);
                                write_ship.update(|s| s.crew_size = v);
                            }
                        />
                    </div>
                    <div>
                        ...
                        <input
                            ...
                            min="0"
                            prop:value=move || ship.with(|s| s.crew_staterooms)
                            on:change=move |ev| {
                                let v: i32 = event_target_value(&ev).parse().unwrap_or(0).max(0);
                                write_ship.update(|s| s.crew_staterooms = v);
                            }
                        />
                    </div>
                </div>
                <div class="skill-entry">
                    <div>
                        ...
                        <input
                            ...
                            min="0"
                            prop:value=move || ship.with(|s| s.passenger_staterooms)
                            on:change=move |ev| {
                                let v: i32 = event_target_value(&ev).parse().unwrap_or(0).max(0);
                                write_ship.update(|s| s.passenger_staterooms = v);
                            }
                        />
                    </div>
                    <div>
                        ...
                        <input
                            ...
                            id="ship-low-berths"
                            min="0"
                            prop:value=move || ship.with(|s| s.low_berths)
                            on:change=move |ev| {
                                let v: i32 = event_target_value(&ev).parse().unwrap_or(0).max(0);
                                write_ship.update(|s| s.low_berths = v);
                            }
                        />
                    </div>
                    <div>
                        ...
                        <input
                            ...
                            min="0"
                            prop:value=move || ship.with(|s| s.cargo_capacity)
                            on:change=move |ev| {
                                let v: i32 = event_target_value(&ev).parse().unwrap_or(0).max(0);
                                write_ship.update(|s| s.cargo_capacity = v);
                            }
                        />
                    </div>
                </div>
                <div class="skill-entry">
                    <div>
                        ...
                        <input
                            ...
                            id="ship-mortgage"
                            min="0"
                            prop:value=move || ship.with(|s| s.mortgage_per_period)
                            on:change=move |ev| {
                                let v: i64 = event_target_value(&ev).parse().unwrap_or(0).max(0);
                                write_ship.update(|s| s.mortgage_per_period = v);
                            }
                        />
                    </div>
                    <div>
                        ...
                        <input
                            ...
                            id="ship-maintenance"
                            min="0"
                            prop:value=move || ship.with(|s| s.maintenance_per_period)
                            on:change=move |ev| {
                                let v: i64 = event_target_value(&ev).parse().unwrap_or(0).max(0);
                                write_ship.update(|s| s.maintenance_per_period = v);
                            }
                        />
                    </div>
                    <div>
                        ...
                        <input
                            ...
                            id="ship-salary"
                            min="0"
                            prop:value=move || ship.with(|s| s.salary_per_period)
                            on:change=move |ev| {
                                let v: i64 = event_target_value(&ev).parse().unwrap_or(0).max(0);
                                write_ship.update(|s| s.salary_per_period = v);
                            }
                        />
                    </div>
                </div>
                <div class="skill-entry">
                    <div>
                        <span>
                            ...
                            <strong>
                                {move || {
                                    Credits::from(ship.with(|s| s.monthly_expenses())).as_string()
                                }}
                            </strong>
                        </span>
                    </div>
                    <div>
                        <button
                            class="blue-button"
                            on:click=move |_| {
                                if let Some(ref c) = client_for_apply {
                                    c.send_apply_monthly_expenses();
                                }
                            }
                        >
                ...
                <div style="display: flex; align-items: center; padding: 10px;">
                    <button
                        ...
                        on:click=move |_| {
                            ...
                            if let Some(ref c) = client_for_regenerate {
                                c.send_regenerate();
                            }
                        }
                    >
            ...
            // Per-trade-leg section: distance to destination, the broker
            // on the planet we're trading with right now, and whether to
            // include illegal goods on this leg. Trade-class readouts for
            // origin and destination travel here too — they're properties
            // of the leg, not the ship.
            <div class:key-region>
                <h3 class="trade-section">"Trade Leg"</h3>
                <div class="skill-entry">
                    <div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            min="0"
                            max="100"
                            value=move || system_broker_skill.get()
                            on:change=move |ev| {
                                write_system_broker_skill
            ...
            <ShipManifestView
                ...
                ship=ship.into()
                system_broker_skill=system_broker_skill.into()
                distance=distance
...
fn ShipManifestView(
    ...
    ship: Signal<Ship>,
    system_broker_skill: Signal<i16>,
        ...
        <div class="output-region">
            ...
            <div class="manifest-summary">
                {move || {
                    ...
                    let current_ship = ship.get();
                    let freight_tons = available_passengers
                        .read()
                        .as_ref()
                    ...
                    let pax_cargo_tons = manifest.passenger_cargo_tons();
                    let total_cargo = goods_tons as f64 + freight_tons as f64
                        + pax_cargo_tons;
                    let total_passengers = manifest.total_passengers_not_low();
                    let total_low = manifest.low_passengers;

                    let cargo_str = if pax_cargo_tons == 0.0 {
                        format!("{} tons", goods_tons + freight_tons)
                    } else {
                        format!("{:.2} tons", total_cargo)
                    };
                    let cargo_over = total_cargo > current_ship.cargo_capacity as f64;
                    let pax_over = total_passengers > current_ship.passenger_staterooms;
                    let low_over = total_low > current_ship.low_berths;
                    let cargo_color = if cargo_over { "red" } else { "" };
                    let pax_color = if pax_over { "red" } else { "" };
                    let low_color = if low_over { "red" } else { "" };

                    view! {
                        <div class="summary-line">
                            "Total Cargo Used: "
                            <strong style:color=cargo_color>
                                {format!(
                                    "{} / {} tons",
                                    cargo_str,
                                    current_ship.cargo_capacity,
                                )}
                            </strong>
                            " | Total Passengers: "
                            <strong style:color=pax_color>
                                {format!(
                                    "{} / {}",
                                    total_passengers,
                                    current_ship.passenger_staterooms,
                                )}
                            </strong>
                            " | Total Low: "
                            <strong style:color=low_color>
                                {format!("{} / {}", total_low, current_ship.low_berths)}
                            </strong>
                        </div>
                    }
                }}
                                ...
                                write_ship_manifest
                                    .update(|manifest| {
                                        ...
                                        manifest
                                            .price_goods(
                                                &origin_world.get(),
                                                ship.with(|s| s.broker_skill),
                                                system_broker_skill.get(),
                                            );
