...
- ALLOWED: jump drive, J-1 through J-6, jump-space / J-space, maneuver drive, gravitic plates, fuel scoops, fuel processors, life support, sandcasters, missile turrets, beam laser, pulse laser, fusion gun, particle accelerator, nuclear damper, meson screen, black globe, pop-up turret, vacc suit, low berth, high passage / middle passage / basic / low passage, Imperium, sector, subsector, parsec, starport classes A-E. For interstellar messaging: x-boat / express boat, x-boat tender, courier drum, data crystal, "drop our packet on the next x-boat run". For in-system messaging: radio, laser comm (both light-speed-limited and only useful within a single star system).
- FORBIDDEN: warp drive, hyperspace, deflector shields, phasers, photon torpedoes, transporters, holodecks, replicators, force fields, lightsabers, "Federation". Anything from Star Trek, Star Wars, Mass Effect, etc. — none of it. Specifically: NO FTL radio, NO subspace radio, NO ansible, NO "beaming" a message between stars, NO instantaneous interstellar communication of any kind. Phrases like "beamed a distress signal toward [other-system world]", "broadcast across the gulf", "sent a transmission to [other-system world]" are WRONG and break the setting — see INTERSTELLAR COMMUNICATION below.
- Money is in credits (Cr) or kilocredits (KCr); large hauls are megacredits (MCr).

== INTERSTELLAR COMMUNICATION ==
This is the rule writers most often get wrong. In Traveller there is NO faster-than-light radio. Every message that travels between star systems is physically carried by a jump-capable ship — there is no other way. The Imperium runs the Express Boat (x-boat) courier network for routine mail; jumps take a week per parsec just like any other ship. In remote areas messages hitch on whatever free trader is heading the right way. A distress call from a marooned ship is RECORDED locally, then HANDED OFF to the next outbound vessel (or x-boat tender at a class-A/B port), which jumps it to the next sector node, which passes it to the next courier, and so on. The "received-by" date in the data is when the physical message is delivered to the home-world starport — NOT a light-speed crossing.
Right register: "we tucked the courier drum into the next outbound x-boat", "bribed a free trader bound for [home] to carry our packet up the line", "the tender's master logged our distress on the manifest before jumping out".
Wrong register (do not use, even with sci-fi flavoring): "we beamed a signal", "transmitted across the void", "broadcast a distress call toward [another star]", "the signal reached [another star] after sixty days".
...
== HOME PORT IS THE START, NOT AN ARRIVAL ==
The first port-visit block in the data is flagged "(HOME PORT — voyage starts here, ship is already in port)". This is the launch, not an arrival — the ship has been on the ground at home all along. Do NOT write "We arrived at [home]" or "We made port at [home]" for the opening; that breaks immersion (we were never away). Frame it as the launch instead: provisioning, the last cargo loaded onto the manifest, signing on or paying off crew, undocking, the first jump outbound. The data line "Voyage launch date" is when the voyage clock starts; "Jumped outbound" is when she lifted. There is no inbound-jump line for the home-start visit because there isn't one. The "(return home)" final visit on the same world IS an arrival — narrate that one normally.
