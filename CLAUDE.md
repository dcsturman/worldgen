...
- `src/worldmap/` — Per-planet surface maps from a UWP + seed. Terrain is sampled on a 3D unit sphere (icosahedral grid + simplex-fBm elevation, climate, biome — see `grid::xy_to_sphere`) and rendered two ways: the **flat** equirectangular SVG/PNG (`render`, `raster::RasterJob`) and a **globe** (`globe`) that orthographically warps a once-built equirectangular texture onto a spinning sphere. The globe ships as a static PNG or a looping animated PNG (APNG) via `render_globe_png` / `render_globe_apng`; the frontend animates live by re-warping the texture on a `<canvas>` each frame. Both projections are deterministic from `(uwp, seed)`.
  Every climate decision (temperature, water, humidity, rain-shadow wind, cloud bands, settlement placement) goes through `climate::ClimateModel`, stored on `WorldMap`, so per-hex biomes and per-pixel flat/globe colour share one rule. `Rotating` is the latitude model and must stay **byte-identical** — `worldmap::golden` pins flat/globe hashes computed before the model existed; if a pin moves, a rotating world changed and every cached one is stale. `TideLocked` replaces latitude with θ, the angle from the substellar point: temperature is SPEC's `cos(θ)^¼` insolation curve blended with an atmospheric-transport term and softened over a twilight band, with `t_sub`/`t_night` set so the world's mean matches the rotating model's (spread by atmosphere code); water fills an antistellar ice sheet first (up to 15% of the surface) and only the excess is liquid, pooled at the inner terminator edge via a θ-biased *water potential* — on a locked world `sea_level` thresholds that potential, not raw elevation; settlements are confined to θ 60–120°, weighted to 75–105°. Resolving a lock draws nothing from the map RNG, so the terrain is the unlocked world's. Globe lighting: a rotating world is lit from a fixed view-space direction (`LIGHT`), but a locked world carries its substellar point on `GlobeTexture::sun` and `warp_into` rotates that sun with the spin, so its day side stays over the substellar point in the APNG, the static globe and the browser globe. WebGL clients light the texture themselves and get the same point from the `X-Substellar` header.
- `src/callisto/` — **Callisto**, the physics-based replacement for Book 6 system generation, in progress on the `callisto` branch (rules: `docs/callisto/source.md`; spec: `docs/callisto/IMPLEMENTATION.md`). The rulebook's tables are data in `docs/callisto/tables.json`, compiled in with `include_str!` (so `.gitignore`, `.dockerignore` and the Dockerfile all name it); prose tables are code constants with a test that each figure is still printed in its cell. `generate::generate_from_constraints_seeded` takes the same `SystemConstraints` Book 6 does and returns the same `System`, with `System::callisto` (a `layout::Layout`) giving every orbit slot its real position in HD, Mkm and zone; Book 6 systems leave it `None` and render exactly as before. Rolls go through `dice::Roller`, so tests replay the rulebook's worked examples with `dice::Scripted`. `api::Generator` selects the rules: the library's plain entry points stay on Book 6; the HTTP server defaults to Callisto, with `generator=book6|callisto` per request (unknown value → 400) and `WORLDGEN_GENERATOR=book6` to flip a whole instance. `callisto::generate::dump::callisto_dump` (ignored) renders sample systems under both for eyeballing. Stage 2 fills orbits (`callisto::populate`): pinned bodies first (Book 6 orbit numbers converted to HD and snapped to the nearest free orbit within ×1.25, else a new orbit), then giants, the ice belt, Table 21, and world codes; `World::callisto` (`body::Physics`: class, composition, gravity, ice) and `GasGiant::callisto` (`GiantKind`) carry the results, and `Layout::fuel` the refuelling line. `SystemConstraints::counts` (`PublishedCounts`, set by `system_from_upstream` from what TravellerMap sent) picks how the rest fill (rulebook Section 6): `All` places the published belts and worlds innermost first, Table 21 deciding only a world's kind, and trims empty orbits past the outermost body (the Book 6 placement). A known count that runs out of orbits adds them per Section 4.4: outward by Table 12 while inside 100 HD and clear of gaps, then by splitting the widest gap between neighbours at their geometric mean (`orbits::split_widest`, ratio ≥ 1.56, companion-gap edges counting as neighbours), and past 100 HD only with a note; `GiantsAndBelts` places the belts, then rolls Table 21 with Belt read as World; `None` rolls everything. A companion's listed bodies (`secondary_bodies`/`tertiary_bodies`) are placed around it; `overrides::apply_post` searches companions to any depth. `validate-overrides -- --callisto` checks the curated systems under Callisto. Stage 3 adds each body's mean temperature and band (`callisto::temperature`, Tables 24–27 with the ice and steam checks) and a `fit` (`body::Fit`): `Strained` when a published fact forces something the physics can't support (a listed giant or white dwarf host, a stated orbit in a companion's unstable zone or one that leaves a habitable world Frozen/Roasting), `Adjusted` when a companion or a pinned orbit moved; Section 13.2's easy-story oddities go in `Physics::oddities` instead. `cargo run --bin callisto-review --features backend -- "<sector>"` writes `docs/callisto/review/<sector>.md` (replaced, deterministic) listing strained worlds, then oddities. The stellar parser now keeps brown dwarfs (`StarSize::BD`) and bare white dwarfs (`D`, subtype `None`) for Callisto; Book 6's `collect_overrides` and the override star-patcher skip them, so Book 6 output is unchanged.
